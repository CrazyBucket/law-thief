...
| 术语 | 当前实现 | 示例 |
| --- | --- | --- |
| 地块 Ground Tile | `TileState.tile_id` | 地板、水、冰、草地、灌木 |
...
覆盖性操作必须在预览结果中明确说明。默认策略为允许覆盖，但使用黄色高亮并记录为一个可撤销命令。
...
当前 `DataRegistry.get_tile_ids()` 暴露地板、水、冰、草地和灌木。
...
- [x] 可随意创建宝石，明确选择单位槽位，并按颜色新增任意数量槽位。
