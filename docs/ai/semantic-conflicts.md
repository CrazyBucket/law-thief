...
- `详细设计/宝石/宝石_v1.md` 将冻结定义为独立状态：消耗潮湿、跳过下一次自身行动、期间承受 25% 额外普通伤害，解冻时有 50% 概率获得冻伤。
- 当前运行时已独立建模 `frozen`，覆盖冻结的行动跳过、普通伤害修正、潮湿消耗、状态图标，以及普通冰宝石和 Boss 冰法术路径；不再用 `paralyzed + slowed` 代替冻结。
- 红色冰冻和黑色冰冻 Lv3 的可执行语义合约已改为断言 `frozen`，并明确排除占位 `paralyzed`。
- 同一设计来源将冻伤定义为承受普通伤害 +25%、普通攻击输出 -25%，但未说明持续时间或移除规则。

默认权威：详细宝石设计。冻结核心语义已实现并覆盖；在产品明确冻伤持续时间与移除规则之前，不创建永久冻伤或猜测其生命周期。

## 潮湿电弧增幅与剧毒反应

- 潮湿电弧原始描述要求“伤害与弹射范围提升”，但没有给出伤害倍率或范围增量；剧毒反应也只给出了“初始层数 +1 或扩散”的候选方向。
- 当前 MVP 将电弧增幅固定为：主目标追加 1 次标准电弧伤害，传导范围取当前电弧范围 `+1`，范围内所有潮湿或脚踩浅水单位各承受 1 次等额电弧；放电者满足条件时也会受击。所有实际持有潮湿的参与者均消耗潮湿，Boss 无例外。
- 剧毒潮湿反应保持预留，不在两个候选方向之间擅自选择；首版也不生成永久通电水域或冰面。

默认权威：本次潮汐机制确认与详细宝石设计。电弧 MVP 数值已作为可执行口径；剧毒反应仍待产品确定。
