...
## 中毒结算时机（运行时已按高优先级来源处理）

- `详细设计/宝石/宝石_v1.md` 的蓝色剧毒 Lv2 明确写“中毒者回合结束时”，`GDD.md` 也将中毒与着火定义为回合结束结算。
- `Technical_Architecture.md` 的旧状态示例则写中毒在回合开始造成伤害。
- 运行时按来源优先级采用“状态持有者自己的行动窗口结束时结算”；场上其他单位不会在该边界被一起结算。状态伤害同时进入战斗事件流，供表现层按结算顺序播放。
