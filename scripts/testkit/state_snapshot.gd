...
static func capture(state: GameState, events: Array = [], include_log: bool = true) -> Dictionary:
	var snapshot := {
		...
		"generation": _json_safe(state.generated_encounter_blueprint.get("generation", {})),
		...
		"split_move": {
			"uid": state.split_move_uid,
			"remaining": state.split_move_remaining,
			"capacity": state.split_move_capacity,
		},
		"result": state.result,
		"units": [],
		"gems": [],
