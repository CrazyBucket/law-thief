...
func _run_tests() -> void:
	...
	assert(int(heal.get("amount", 0)) == 32, "rest heal should apply configured 20 percent plus 12 hp")
	assert(int(heal.get("after_hp", 0)) == 37, "rest heal should report the post-heal health")
