...
static func _death_hook_context(entry: Dictionary, death_chain_id: int) -> Dictionary:
	...
	return {
		...
		"black_death_already_triggered": bool(lethal_damage.get("black_death_already_triggered", false)),
	}
...
static func _drop_enemy_gems_to_ground(state: GameState, unit: UnitState) -> void:
	...
	for unit_slot: SlotState in unit.slots:
		...
		state.log("%s 掉落在 %s" % [gem_name, unit.pos])
