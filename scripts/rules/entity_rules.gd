...
## 单位步入实体格（主动移动）
static func on_unit_entered(state: GameState, unit: UnitState) -> void:
...
static func on_unit_collide_entity(
	...
	match entity.entity_id:
		...
		Constants.ENTITY_SPIKE:
			...
			var dealt := CombatRules.apply_damage(state, unit, Constants.SPIKE_COLLISION_DAMAGE, source_uid, "spike_collision")
			if dealt > 0 and events != null:
				events.append({"type": "damage", "pos": unit.pos, "damage": dealt, "is_crit": true})
			return true
