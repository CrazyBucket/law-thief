...
static func _balance_int(unit_def_id: String, key: String) -> int:
	...
	if registry == null:
		push_error("PatrolGuardRules: DataRegistry unavailable")
		return 0
	var value: Variant = registry.get_unit_balance_value(unit_def_id, key, null)
	if value == null:
		push_error("PatrolGuardRules: required balance missing: %s.%s" % [unit_def_id, key])
		return 0
	return int(value)
...
static func rampage_move_points(unit: UnitState) -> int:
	return unit.move_points + _balance_int(unit.unit_def_id, "rampage_move_bonus")
...
static func charge_bonus(state: GameState, unit: UnitState, move_start_pos: Vector2i, path: Array) -> int:
	...
	var charge_min_steps := _balance_int(unit.unit_def_id, "charge_min_steps")
	...
	if _charge_move_cost_sum(state, charge_cells) > charge_min_steps:
		return 0
	return _balance_int(unit.unit_def_id, "charge_bonus")
...
static func melee_damage_preview(
	...
	var base_damage := CombatRules.attack_damage(state, unit) + charge_bonus(state, unit, move_start_pos, path)
